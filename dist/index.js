...
import { createOctokit, fetchPullRequestDiff, getDiffLineNumbers } from "./diff.js";
...
async function main() {
    ...
    if (eventName === "issue_comment") {
        ...
        const command = typeof comment?.body === "string"
        ...
        if (!isTrustedRereviewActor(comment?.author_association, comment?.user?.type)) {
    ...
    const headlineSummary = summary.summary.startsWith("## swarm-review")
        ? summary.summary
        : `## swarm-review\n\n${summary.summary}`;
    const baseCommentBody = swarmConfig.output.mode === "full"
    ...
    let actualEvent = resolveReviewEvent(reviewEvent, acceptedFindings, getBudgetStatus().exhausted);
    ...
    if (isInline || reviewEvent !== "COMMENT") {
        ...
        await createPullRequestReview(octokit, owner, repo, pullNumber, actualEvent, reviewBody, reviewComments);
    }
